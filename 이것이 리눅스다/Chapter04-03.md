...
2. DVD 마운트
   ...
   우선 마운트의 개념이 처음이라 약간의 오개념이 존재 할 수도 있습니다.

   windows 같은 경우 USB나 CD/DVD를 넣었을 때 자동으로 인식하여 드라이브를 할당받지만 linux에서는 수동으로 연결해 주어야 합니다.  
   즉 마운트란 특정 디바이스(device)를 사용하기 위해 하드웨어장치와 디렉토리를 연결하는 작업을 의미합니다.

   [실습6] cd 마운트하기, 마운트된 cd로 들어가보기, 언마운트 하기

   CD를 마운트 해제하려면 ```umount  /dev/cdrom```
   
   CD를 넣고 ```monut```를 입력하면 마지막에 ```/dev/sr0```로 시작하는 파일이 추가됩니다.(VMware가 자동으로 마운트를 해버림)
   ...
   CD의 디렉토리로 들어가려면 위 결과의 두번째 빨간창으로 들어가면 됩니다.

   따라서 ```cd  /run/media/root/CentOS(자동완성 사용)```으로 들어가면 cd의 경로로 들어갑니다.

   작업이 끝난 후에는 반드시 cd를 빠져나와서 umount를 진행 하여야 합니다.
